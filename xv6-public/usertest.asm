...
_usertest:     file format elf32-i386
...
int main(int argc, char *argv[]){
  ...
  getpinfo(&procstat);
  13:	8d b5 e8 fb ff ff    	lea    -0x418(%ebp),%esi
...
int main(int argc, char *argv[]){
  ...
  30:	e8 5e 03 00 00       	call   393 <getpinfo>
  ...
  43:	ff b4 9e 00 03 00 00 	pushl  0x300(%esi,%ebx,4)
  4a:	ff b4 9e 00 02 00 00 	pushl  0x200(%esi,%ebx,4)
  51:	ff b4 9e 00 01 00 00 	pushl  0x100(%esi,%ebx,4)
  58:	53                   	push   %ebx
  for(int i = 0; i < NPROC; i++){
  59:	83 c3 01             	add    $0x1,%ebx
    printf(1, "procnum %d [tickets %d][pid %d][ticks %d]\n", i, procstat.tickets[i], procstat.pid[i], procstat.ticks[i]);
  5c:	68 b8 07 00 00       	push   $0x7b8
  61:	6a 01                	push   $0x1
  63:	e8 e8 03 00 00       	call   450 <printf>
  for(int i = 0; i < NPROC; i++){
  68:	83 c4 20             	add    $0x20,%esp
  6b:	83 fb 40             	cmp    $0x40,%ebx
...
printf(int fd, const char *fmt, ...)
{
 ...
 5d4:	eb 1a                	jmp    5f0 <printf+0x1a0>
          s = "(null)";
 5d6:	bb e3 07 00 00       	mov    $0x7e3,%ebx
        while(*s != 0){
